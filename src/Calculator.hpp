//TODO
/* Pseudocode 
- Receives parameters from IOManager as string
- Calculate according to the number base
- Pass result to IOManager as a string
- Calculations for every base (2, 10, 16)
    +, -, *, /, %, ^, sqrt
 */

// returns result of num2 added to num1
float calculate_add(float num1, float num2);

// returns result of num2 substracted from num1
float calculate_substract(float num1, float num2);

// returns result of num1 multiplied by num2
float calculate_multiply(float num1, float num2);

// returns result of num1 divided by num2
float calculate_divide(float num1, float num2);

// returns reminder from division of num1 by num2
float calculate_modulo(int num1, int num2);
